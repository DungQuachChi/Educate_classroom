...
class _QuizFormScreenState extends State<QuizFormScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      quizProvider.loadQuestionsByCourse(widget.courseId);

      _countAvailableQuestions();
    });
    ...
    if (isEditing) {
      ...
      _mediumCountController.text = widget.quiz!.structure.mediumCount.toString();
  ...
  void _countAvailableQuestions() {
    final quizProvider = Provider.of<QuizProvider>(context, listen: false);
    final questions = quizProvider.questions;

    setState(() {
      _availableEasy = questions.where((q) => q.difficulty == QuestionDifficulty.easy).length;
      _availableMedium = questions.where((q) => q.difficulty == QuestionDifficulty.medium).length;
      _availableHard = questions.where((q) => q.difficulty == QuestionDifficulty.hard).length;
    });
  }
  ...
  Future<void> _selectOpenDate() async {
    ...
    if (picked != null) {
      setState(() {
        _openTime = DateTime(picked.year, picked.month, picked.day, _openTimeOfDay.hour, _openTimeOfDay.minute);
      });
    }
  }

  Future<void> _selectOpenTime() async {
    final TimeOfDay? picked = await showTimePicker(context: context, initialTime: _openTimeOfDay);
    if (picked != null) {
      setState(() {
        _openTimeOfDay = picked;
        _openTime = DateTime(_openTime.year, _openTime.month, _openTime.day, picked.hour, picked.minute);
      });
    }
  }
  ...
  Future<void> _selectCloseDate() async {
    ...
    if (picked != null) {
      setState(() {
        _closeTime = DateTime(picked.year, picked.month, picked.day, _closeTimeOfDay.hour, _closeTimeOfDay.minute);
      });
    }
  }

  Future<void> _selectCloseTime() async {
    final TimeOfDay? picked = await showTimePicker(context: context, initialTime: _closeTimeOfDay);
    if (picked != null) {
      setState(() {
        _closeTimeOfDay = picked;
        _closeTime = DateTime(_closeTime.year, _closeTime.month, _closeTime.day, picked.hour, picked.minute);
      });
    }
  }
  ...
  Future<void> _save() async {
    ...
    if (easyCount > _availableEasy) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Not enough easy questions (available: $_availableEasy)'),
          backgroundColor: Colors.red,
        ),
      );
      return;
    ...
    if (mediumCount > _availableMedium) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Not enough medium questions (available: $_availableMedium)'),
          backgroundColor: Colors.red,
        ),
      );
      return;
    ...
    if (hardCount > _availableHard) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Not enough hard questions (available: $_availableHard)'),
          backgroundColor: Colors.red,
        ),
      );
      return;
    ...
    try {
      ...
      final questionIds = await quizProvider.selectRandomQuestions(widget.courseId, structure);
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(isEditing ? 'Quiz updated' : 'Quiz created')),
        );
      }
    } catch (e) {
      if (mounted) {
          ...
          children: [
            ...
            Card(
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    const Text('Open Time *', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                      ...
                      children: [
                        Expanded(
                          child: OutlinedButton.icon(
                            ...
                            label: Text(DateFormat('MMM dd, yyyy').format(_openTime)),
                          ),
                        ),
            ...
            Card(
              child: Padding(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text('Close Time *', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                    const SizedBox(height: 12),
                    Row(
                      children: [
                        Expanded(
                          child: OutlinedButton.icon(
                            ...
                            label: Text(DateFormat('MMM dd, yyyy').format(_closeTime)),
                          ),
                        ),
                        const SizedBox(width: 12),
                        Expanded(
            ...
            Card(
              child: Padding(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text('Quiz Structure', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                    const SizedBox(height: 8),
                    const Text('Questions will be randomly selected', style: TextStyle(fontSize: 12, color: Colors.grey)),
                    const SizedBox(height: 16),
                        ...
                        Expanded(
                          child: TextFormField(
                            ...
                            decoration: InputDecoration(
                              ...
                              border: const OutlineInputBorder(),
                              suffixText: '/ $_availableMedium',
                              prefixIcon: Icon(Icons.circle, color: Colors.orange, size: 12),
                            ),
                            keyboardType: TextInputType.number,
                            validator: (value) {
                              if (value == null || value.isEmpty) return 'Required';
            ...
            Card(
              child: Padding(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text('Quiz Options', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                    SwitchListTile(
                    ...
                    SwitchListTile(
                      ...
                      subtitle: const Text('Shuffle A, B, C, D for each student'),
            ...
            ElevatedButton(
              ...
              child: _isSaving
                  ...
                  : Text(isEditing ? 'UPDATE QUIZ' : 'CREATE QUIZ', style: const TextStyle(fontSize: 16)),
            ),
          ],
        ),
      ),
